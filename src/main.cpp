...
namespace {
const std::string DATA_FILE = "data/inventory.csv";
...
int readInt(const std::string& prompt, int minimum = 0) {
    int value;
    ...
    while (true) {
        ...
        std::cout << "Please enter a whole number greater than or equal to " << minimum << ".\n";
        clearInput();
    }
}

double readDouble(const std::string& prompt, double minimum = 0.0) {
    double value;
    ...
    while (true) {
        ...
        std::cout << "Please enter a number greater than or equal to " << minimum << ".\n";
        clearInput();
    }
}
...
void printItemTable(const std::vector<Item>& items) {
    ...
    for (const Item& item : items) {
        std::cout << std::left
                  ...
                  << std::setw(14) << item.getReorderLevel()
                  << std::setw(14) << item.getTotalValue()
                  << '\n';
    }
}
